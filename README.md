...
Find the built executable in `./target/release/axum-dev`. You can
...
[axum-dev/Cargo.toml](axum-dev/Cargo.toml).
 ...
 * [Download the latest release](https://github.com/enigmacurry/axum-dev/releases/latest)

Docker images built for X86_64 and AArch64 are published on the GitHub
container registry (`ghcr.io`):

 * [Pull the latest Docker image](https://github.com/enigmacurry/axum-dev/pkgs/container/axum-dev)
...
files in `${XDG_DATA_HOME}/axum-dev`, or `${HOME}/.local/share/axum-dev`
(if no `XDG_DATA_HOME` is set) or `./axum-dev-data` (if no `HOME`
variable is set).
...
Run `axum-dev --help` to find all of the available options. Here are a
...
source <(axum-dev completions bash)
```
...
source <(axum-dev completions zsh)
```
...
axum-dev completions fish > ~/.config/fish/completions/axum-dev.fish
```
